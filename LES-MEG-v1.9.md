# GameHub 1.9 — statistikk, åpningsanimasjon flyttet, mykere lyd

## Hva som er nytt i 1.9

**Bruksstatistikk til nettsiden.** Appen sender hvert femte minutt en liten
anonym melding til `webgamehubweb.vercel.app/api/ping`: en tilfeldig
install-ID (lages første gang), versjon, språk, om vinduet er åpent eller i
systemkurven, spillet som spilles akkurat nå, antall installerte spill og
hvilke launchere, Windows-versjon, og hvor mange ganger hver funksjon er
brukt siden sist (spill startet, screenshot, klipp, klipp til, frys,
bakgrunnsbilde, Hurtigverktøy). Ingen navn, ingen filstier, ingen IP.
Sendingen kan ikke slås av, men det står under Innstillinger → Personvern
hva som sendes. Dashbordet ligger på nettsiden (`/admin`) — se
`OPPSKRIFT-NETTSIDE.md`. Meldingen har ingen betydning for appen: feiler
den, prøves det igjen om fem minutter, og tallene tas med da.

**Åpningen er flyttet** til Innstillinger → Generelt → «Når GameHub åpner»
(lå under Utseende). Lyden er lagd på nytt: en oktav dypere, mykere
anslag og lavere volum (`GameHub-startup-sound-v2.mp3`).
...
Så selve opplastingen. `gamehubv27.zip` inneholder alt. GitHub tar imot
maks 100 filer per opplasting, og `apps`-mappen alene er 105 filer —
derfor tre runder:

1. Pakk ut `gamehubv27.zip`. Gjør skjulte filer synlige (Del 2 i
...
5. Gå til **Actions**-fanen. Fordi byggingen startet i runde 2, før
   ...
   workflow**. Den bygger nå 1.9.0 med alt.

Sjekk at det gikk riktig: `apps/desktop/src/Splash.tsx` skal finnes på
GitHub, og `apps/desktop/src-tauri/tauri.conf.json` skal si `"1.9.0"`.
