...
**The documents users read are the markdown files in this folder.** The app
compiles them in (`apps/desktop/src/legal.ts` imports them), and the website
reads the same files from GitHub. One copy of each, so the terms someone
accepted are exactly the ones that shipped in their version — two versions of
a privacy policy that disagree is itself a legal problem.

The rest of this folder is the working papers behind them: the analysis, the
record of processing, the risk register and the launch checklist.
