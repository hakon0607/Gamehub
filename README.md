...
- **The assistant.** The settings are there and a key can be stored, but the
  panel inside the app is not built and nothing is sent anywhere.
- **Cloud sync.** Deliberately not built. GameHub keeps everything on the PC it
  runs on, which is also why it needs no account.
...
├── legal/                  Terms, privacy policy and the compliance papers
├── packages/
...
1. Replace `OWNER` in `tauri.conf.json` with your GitHub username.
2. For auto-update: follow `docs/UPDATES.md`. Three repository secrets, and the
   release workflow does the rest — an updater without a signing key is worse
   than none.
3. Read `legal/LAUNCH-CHECKLIST.md` before handing the installer to anyone
   else. It covers the terms, the consent screen and what has to be true
